...
def process_file(file_name):
    try:
        ...
        # Check again before processing the file
        if redisClient.zscore("channel_downloaded", id) is not None:
        ...
        # Redis distributed lock to avoid race condition
        with redisClient.lock(f"lock:{id}", blocking_timeout=5):
